#!/usr/bin/python
'''
This purpose of this program is to provide a light weighted tool to monitor Lustre storage's performance.
It only uses ssh, lctl command, python's curses, multiprocess modules.
Lustre seems has very poor manual for it's procfs system, many of them are not clear what they really are. We can still get 
